...
hal_timer_init(int timer_num, void *vcfg)
{
    ...
    if (!nvic_configured) {
        ...
        NVIC_SetVector(CTIMER_IRQn, (uint32_t)apollo3_timer_isr);
...
hal_timer_delay(int timer_num, uint32_t ticks)
{
    ...
    until = apollo3_timer_cur_ticks(bsp_timer) + ticks;
    while ((int32_t)(apollo3_timer_cur_ticks(bsp_timer) - until) <= 0) { }
...
hal_timer_start(struct hal_timer *timer, uint32_t ticks)
{
    ...
    cur = apollo3_timer_cur_ticks(bsp_timer);
    exp = ticks + cur;
